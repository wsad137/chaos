...
public class AccountManager {
    ...
    public String shortUrl(String longUrl) {
        ...
        String resultStr = HttpUtils.post(SHORTURL_POST_URL + TokenProxy.accessToken(config), requestData);
    ...
    private Qrcode createQrcodeTicket(QrcodeType qrcodeType, Integer expireSeconds, Long sceneId, String sceneStr) {
        ...
        logger.info("request data " + requestData);
        String resultStr = HttpUtils.post(QRCODE_POST_URL + TokenProxy.accessToken(config), requestData);
        logger.info("return data " + resultStr);
        try {
            WeChatUtil.isSuccess(resultStr);
        } catch (WeChatException e) {
